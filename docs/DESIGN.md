...
- Don't put pill `{rounded.full}` radii on form fields — inputs stay tight at `{rounded.xs}` (4px) or `{rounded.md}` (8px).
...
## 14. EduHub LMS Component Mappings

### 1. Course Cards (`/courses`, `/me/courses`, `/me/enrollments`)
- Surface: Pure white `{colors.surface}` (#ffffff), border 1px `{colors.hairline}` (#e6e6e6), rounded `{rounded.lg}` (12px).
- Hover: Soft Level-1 micro-shadow (`rgba(0,0,0,0.04) 0 4px 18px`), image thumbnail slightly elevated.
- Category / Level Badges: Sticker palette pills (`{colors.accent-purple}`, `{colors.accent-teal}`, `{colors.accent-orange}`, `{colors.accent-sky}`) with light tinted backgrounds and dark text.
- Progress Bar: Background `{colors.hairline}` with `{colors.primary}` (#0075de) active fill.

### 2. Learning Video Player (`/learn/:slug/:lessonId`)
- Container: `{colors.surface-dark}` (#101010) background, 16:9 ratio, 1px border `{colors.hairline}`.
- Playback & Heartbeat: Progress indicator uses `{colors.primary}` (#0075de) with white scrub thumb.
- Sidebar Lesson Tree: Warm canvas `{colors.canvas-soft}`, active lesson highlighted with white card surface + blue active bar indicator (`{colors.primary}`).

### 3. Publish Checklist (`BR-CRS-02`)
- Surface: White card `{colors.surface}` with Level-1 soft shadow.
- Checklist Items: Valid items use `{colors.accent-green}` CheckCircle2; invalid items use `{colors.accent-orange}` or Error XCircle with a small `{components.button-utility}` action.
- Publish Button: Pill `{components.button-primary}` (#0075de), disabled when checklist is incomplete.

### 4. Headless Tables (`/admin/*`, `/teacher/*`)
- Table Header: `{colors.canvas-soft}` (#f6f5f4) background, `{typography.eyebrow}` (12px / 600 uppercase), border bottom 1px `{colors.hairline}`.
- Row: 48px height, hover tint `{colors.canvas-soft}`, bottom border `{colors.hairline}`.
- Role Badges:
  - `STUDENT`: `{colors.canvas-soft}` border `{colors.hairline}` text `{colors.ink-secondary}`.
  - `TEACHER`: `{colors.accent-purple}` tint text `{colors.accent-purple-deep}`.
  - `ADMIN`: `{colors.accent-orange}` tint text `{colors.accent-orange-deep}`.
