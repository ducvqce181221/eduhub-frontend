...
export function CourseCardSkeleton() {
  return (
    <div className="flex flex-col rounded-xl bg-surface border border-hairline overflow-hidden shadow-notion-soft animate-pulse">
      {/* Thumbnail skeleton */}
...
export function CourseGrid({
  ...
  if (courses.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4 text-center rounded-2xl border border-dashed border-hairline bg-surface/50">
        <div className="w-14 h-14 rounded-2xl bg-canvas-soft border border-hairline flex items-center justify-center text-ink-muted mb-4 shadow-2xs">
          {hasActiveFilters ? (
            <SearchX className="w-7 h-7 text-ink-muted" />
          ) : (
            <BookOpen className="w-7 h-7 text-notion-blue" />
          )}
        </div>
